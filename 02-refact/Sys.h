...
private:
	...
	void EventBorn() {
		OnBorn(*this);
		for (auto prop_event : prop_events["Born"]) prop_event();
	}
	void EventStep() {
		OnStep(*this);
...
struct Sys {
	...
	inline void RenderText(ID3DX10Font* font, std::wstring text, f2 pos, f4 color = { 0,0,0,1 }) {
		RECT rect = {
			pos.x, pos.y,
			0, 0
		};
		font->DrawTextW(nullptr, text.c_str(), text.size(), &rect, DT_NOCLIP, D3DXCOLOR((float*)&color));
	}
};
extern Sys* sys;
inline Texture operator"" _tex(const char* raw_path, size_t n) {
